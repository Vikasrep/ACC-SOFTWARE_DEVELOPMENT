...
import { apiRequestIsValid, getReportsAndCharts as apiGetReportsAndCharts } from '@apis';

const slice = createSlice({
	name: 'reportsAndCharts',
	initialState: {
		action: null,
		data: null,
		error: null,
...
export const getReportsAndCharts = (table, params) => async (dispatch) => {
	if (!table) throw Error('table param is required');

	try {
		dispatch(fetching());

		apiGetReportsAndCharts(table, params).then((response) => {
